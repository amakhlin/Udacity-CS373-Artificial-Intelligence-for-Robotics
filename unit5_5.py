...
def smooth(path, weight_data = 0.5, weight_smooth = 0.1):
    ...
    #while(1):
        #util1_max = [0 for i in range(len(path[0]))]
        #util2_max = [0 for i in range(len(path[0]))]
        #u1 = [0 for i in range(len(path[0]))]
        #u2 = [0 for i in range(len(path[0]))]
        
        for i in range(1,len(path)-1):  #points
            for j in range(len(path[0])):   # x, y
                newpath[i][j] = newpath[i][j] + weight_data * (path[i][j] - newpath[i][j])
                #u1[j] = (path[i][j] - newpath[i][j])**2
                #if(u1[j] > util1_max[j]):
                #    util1_max[j] = u1[j]
                #if(i > 1):
                newpath[i][j] = newpath[i][j] + weight_smooth * (newpath[i+1][j] + newpath[i-1][j] - 2.*newpath[i][j])
                #u2[j] = (newpath[i][j] - newpath[i+1][j])**2
                #if(u2[j] > util2_max[j]):
                #    util2_max[j] = u2[j]
        
        #print util1_max, util2_max
        #if(util1_max < .000001 and util2_max < .000001):
        #    break;
        #print util1_max[0], util2_max[0], util1_max[1], util2_max[1]
