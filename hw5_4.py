...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        D = 0.
        cte = 0.
        if(self.x <= radius):   # left half
            D =  sqrt((radius - self.x)**2 + (self.y - radius)**2)
            cte = D - radius
        elif(self.x > radius and self.x < 3.*radius):   # middle
            if( self.orientation < pi/2. or self.orientation > 3.*pi/2. ): # upper half
                cte = self.y - 2. * radius
            else:
                cte = -self.y
        else:
