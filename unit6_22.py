...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Xi.value[0][0] = world_size / 2. / motion_noise
    Xi.value[1][0] = world_size / 2. / motion_noise
       
    for i in range(N-1):    # there are N-1 points, 1...N
        # go through the data
        ...
        for j in range(len(meas_arr)):
            ...
            # measurement x at i th location
            row_list = [2*i, 2*N + 2*meas_index]
            col_list = row_list
            
            Omega += defOmegaMeas.expand(m_size, m_size, row_list, col_list)
            Xi += (defXiMeas * matrix([[meas_dx]])).expand(m_size,1,row_list,[0])
                     
            # measurement y
            row_list = [2*i + 1, 2*N + 2*meas_index + 1]
            col_list = row_list
            
            Omega += defOmegaMeas.expand(m_size, m_size, row_list,  col_list)
            Xi += (defXiMeas * matrix([[meas_dy]])).expand(m_size,1,row_list,[0])
        ...
        row_list = [2*i, 2*(i+1)]
        col_list = row_list
        Omega += defOmegaMotion.expand(m_size, m_size, row_list, col_list)
        Xi += (defXiMotion * matrix([[dx]])).expand(m_size,1,row_list,[0])

        # motion y contraint
        row_list = [2*i + 1, 2*(i+1) + 1]
        col_list = row_list
        Omega += defOmegaMotion.expand(m_size, m_size, row_list, col_list)
        Xi += (defXiMotion * matrix([[dy]])).expand(m_size,1,row_list,[0])
...
#result = slam(test_data1, 2, 2, 1.0, 1.0)
#print_result(2, 2, result)
#print result
