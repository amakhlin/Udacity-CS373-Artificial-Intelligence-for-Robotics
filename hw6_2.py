...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    m_size = 2 + 2*num_landmarks
    m_size_e = m_size + 2
    
    Omega = matrix()
    Omega.zero(m_size, m_size)
    Xi = matrix()
    Xi.zero(m_size, 1)
   
    defOmegaMeas = matrix([[1./measurement_noise, -1./measurement_noise],
                     [-1./measurement_noise, 1./measurement_noise]])
    defXiMeas = matrix([[-1./measurement_noise],[1./measurement_noise]])
    
    defOmegaMotion = matrix([[1./motion_noise, -1./motion_noise],
                     [-1./motion_noise, 1./motion_noise]])
    defXiMotion = matrix([[-1./motion_noise],[1./motion_noise]])
    ...
    for i in range(N-1):    # there are N-1 points
        # expand Omega and Xi
        row_list = [0, 1] + range(4, 4 + 2*num_landmarks);
        col_list = row_list
        Omega = Omega.expand(m_size_e, m_size_e, row_list, col_list)
        Xi = Xi.expand(m_size_e, 1, row_list, [0])
        # go through the data
        
        # unpack data
        data_at_location = data[i]
        meas_arr = data_at_location[0]
        for j in range(len(meas_arr)):
            # measurement contraints
            meas_index = meas_arr[j][0]
            meas_dx = meas_arr[j][1]
            meas_dy = meas_arr[j][2]
            # measurement x at 1 st location
            row_list = [0, 4 + 2*meas_index]
            col_list = row_list
            
            Omega += defOmegaMeas.expand(m_size_e, m_size_e, row_list, col_list)
            Xi += (defXiMeas * matrix([[meas_dx]])).expand(m_size_e,1,row_list,[0])
                     
            # measurement y
            row_list = [1, 4 + 2*meas_index + 1]
            col_list = row_list
            
            Omega += defOmegaMeas.expand(m_size_e, m_size_e, row_list,  col_list)
            Xi += (defXiMeas * matrix([[meas_dy]])).expand(m_size_e,1,row_list,[0])
            
        # motion contraint
        dx = data_at_location[1][0]
        dy = data_at_location[1][1]
       
        # motion x contraint
        row_list = [0, 2]
        col_list = row_list
        Omega += defOmegaMotion.expand(m_size_e, m_size_e, row_list, col_list)
        Xi += (defXiMotion * matrix([[dx]])).expand(m_size_e,1,row_list,[0])

        # motion y contraint
        row_list = [0 + 1, 2 + 1]
        col_list = row_list
        Omega += defOmegaMotion.expand(m_size_e, m_size_e, row_list, col_list)
        Xi += (defXiMotion * matrix([[dy]])).expand(m_size_e,1,row_list,[0])
        
        # fold back
        A = Omega.take([0, 1], range(2, m_size_e))
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        Omega_prime = Omega.take(range(2 , m_size_e), range(2, m_size_e))
        Xi_prime = Xi.take(range(2 , m_size_e), [0])
