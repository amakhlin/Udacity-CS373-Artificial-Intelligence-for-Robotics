...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        d = random.gauss(motion[1], self.distance_noise)      # distance moved
    ...
    # sense: 
    #    
    def sense(self, compute_with_noise = 1):
        Z = []

        for i in range(len(landmarks)):
            gamma = atan2((landmarks[i][0] - self.y), (landmarks[i][1] - self.x))
            bearing = (gamma - self.orientation) % (2.*pi);
            if(compute_with_noise == 1):
